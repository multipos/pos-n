    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(
                      tr("Enter a PoS-N Coin address (e.g. 1zCDq6drGXfBgkt7F3dnofdG822Ymiw9Hq)"));
    ...
    ui->comboCurrency->addItem(COLOR_NAME[PSN_COLOR_P02],
                               (int) PSN_COLOR_P02);
...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPoSNCoin(int)), this, SLOT(updateDisplayUnit()));
        connect(model->getOptionsModel(), SIGNAL(defaultColorChanged(int)), this, SLOT(currencyChanged(int)));

        // Coin Control
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPoSNCoin(int)), this, SLOT(coinControlUpdateLabels()));
        connect(model->getOptionsModel(), SIGNAL(coinControlFeaturesChanged(bool)), this, SLOT(coinControlFeatureChanged(bool)));
        connect(model->getOptionsModel(), SIGNAL(transactionFeeChangedPoSNCoin(qint64)), this, SLOT(coinControlUpdateLabels()));
