...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog, int nColor)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount >= 0)
        {
            ...
            if (nFeeColor == nColor)
            {
                ...
                if ((nPayFee < minTxFee) && (nChange > 0) && (nChange < GetCent(nColor)/10))
                {
                    if (nChange < GetCent(nColor)/10) // change < 0.01 => simply move all change to fees
                    {
                        nPayFee += nChange;
                        nChange = 0;
                    }
                    ...
                    {
                        nChange = nChange + nPayFee - GetCent(nColor);
                        nPayFee = GetCent(nColor);
                    }  
                }
                if (nChange == 0)
                {
    ...
    l8->setStyleSheet((nChange > 0 && nChange < GetCent(nColor)) ? "color:red;" : ""); // Change < 0.01BTC
        
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, GetCent(nColor), nColor)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, GetCent(nColor), nColor)));
    ...
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, GetCent(nFeeColor), nFeeColor)).arg(BitcoinUnits::formatWithUnit(nDisplayUnit, MIN_TX_FEE[nFeeColor], nFeeColor)));
