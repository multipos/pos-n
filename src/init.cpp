...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("posncoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  posncoind [options]                     " + "\n" +
                  "  posncoind [options] <command> [params]  " + _("Send command to -server or posncoind") + "\n" +
                  "  posncoind [options] help                " + _("List commands") + "\n" +
                  "  posncoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "posncoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("posncoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("posncoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: posncoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: posncoind.pid)") + "\n" +
        ...
        "  -defaultcurrency=<ticker>   " + _("Default: PSN") + "\n" +
        "  -defaultstake=<ticker>   " + _("Default: PSN") + "\n" +
        ...
        "  -mineonly=<ticker>" + _("Mine only this currency") + "\n" ;

    return strUsage;
}

const int PSN_COLOR_UNIH = PSN_COLOR_P05;
const int PSN_COLOR_PREM = PSN_COLOR_P01;
const int PSN_COLOR_DPST = PSN_COLOR_P09;
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    assert ((int) PSN_COLOR_END == N_COLORS);
    ...
    // set the number of staking currencies
    {
        ...
        for (int nColor = 1; nColor < N_COLORS; ++nColor)
        {
             ...
             if (nMintColor != (int) PSN_COLOR_P00)
             {
    ...
    else if ((nDefaultStake == PSN_COLOR_P00) && (nNumberOfStakingCurrencies > 1))
    {
        std::string strDefaultStake = GetArg("-defaultstake", COLOR_TICKER[PSN_COLOR_P00]);
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  posncoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "posncoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("posncoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
...
/* Reserve Balances */
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of posncoin") << "\n";
        }
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart posncoin to complete") << "\n";
