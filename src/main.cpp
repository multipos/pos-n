...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 14);
...
unsigned int nStakeMaxAgeTestNet = 60 * 20; // 20 minutes
...
const string strMessageMagic = "PoS-N Coin Signed Message:\n";

// Settings

// nDefaultCurrency and nDefaultStake are for sideways compatibility.
int nDefaultCurrency = (int) PSN_COLOR_P00;
...
int nDefaultStake = (int) PSN_COLOR_P00;
...
bool CTransaction::IsDeposit(MapPrevTx const *pinputs) const
{
    ...
    // test marker CTxOuts
    if ((vout[0].nColor == PSN_COLOR_PREM) && (vout[1].nColor == PSN_COLOR_DPST))
    {
        // test rest of vout
        for (unsigned int i = 2; i < vout.size(); ++i)
        {
            if (vout[i].nColor != PSN_COLOR_DPST)
            {
                return false;
            }
        }
        ...
        if (pinputs)
        {
            ...
            for (unsigned int i = 0; i < vin.size(); ++i)
            {
                ...
                if ((nColor != PSN_COLOR_PREM) && (nColor != PSN_COLOR_UNIH))
                {
                    return false;
                }
            }
...
bool CTransaction::IsWithdrawal(MapPrevTx const *pinputs) const
{
    ...
    // TODO: reverse this test to make function easier to follow
    // test marker CTxOut
    if ((vout[0].nColor == PSN_COLOR_DPST) &&
        ...
        vout[0].IsOpReturn())
    {
        ...
        {
            if (vout[i].nColor != PSN_COLOR_UNIH)
            {
                return false;
            }
        }
                                ...
                                enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if (nMinFee < nBaseFee)
    {
        for (unsigned int i = 0; i < vout.size(); ++i)
        {
            ...
            // withdrawal: vout[0] is burn 0 DPT marker, also meaningles
            if ((i==0) && (IsDeposit() || IsWithdrawal()))
            {
                continue;
            }
            if (vout[i].nValue < GetCent(nColor))
            {
                nMinFee = nBaseFee;
            }
        }
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        {
            nFees = tx.GetValueIn(mapInputs, PSN_COLOR_UNIH) - tx.GetValueOut(PSN_COLOR_DPST);
            if (fDebug)
            {
        ...
        else if (tx.IsWithdrawal(&mapInputs))
        {
            ...
            nFees = tx.GetMaxWithdrawal(mapInputs) - tx.GetValueOut(PSN_COLOR_UNIH);
            if (fDebug)
            {
...
struct AMOUNT GetPoWSubsidy(const CBlockIndex *pindexPrev, int nColor)
{
    ...
    stSubsidy.nValue = 0;
    stSubsidy.nColor = nColor;

    int nHeight = pindexPrev->nHeight + 1;

    if (nHeight <= nLastFairLaunchBlock)
    {
        // fair launch blocks are 1 coin
        stSubsidy.nValue = GetMaxPoWSubsidy(nColor, GetCoin(nColor));
    }
    else if (nHeight <= nLastPoWBlock)
    {
       int64_t nReward = GetMaxPoWSubsidy(nColor, STANDARD_REWARD);
       int64_t d = GetMaxPoWMint(nColor) - pindexPrev->vTotalMint[nColor];
       stSubsidy.nValue = std::min(nReward, d);
    }
    else
    {
       stSubsidy.nValue = 0;
...
struct AMOUNT GetProofOfWorkReward(const CBlockIndex* pindexPrev, int nSubsidyColor)
{
    struct AMOUNT stSubsidy = GetPoWSubsidy(pindexPrev, nSubsidyColor);
...
// fees get eaten by network for PoS until money supply gets near MAX_MONEY
struct AMOUNT GetProofOfStakeReward(const CBlockIndex* pindexPrev, int nStakeColor)
{
    static const int BPY = 262980;    // blocks per year @ 120s
    static const int BP3Y = 3 * BPY;  // blocks per 3 years
    static const int LTAPR = 4;       // 4% long term APR (before compounding)

    int nMintColor = (int) MINT_COLOR[nStakeColor];
    ...
    int nAPR;  // annual percentage rate (before compounding)

    if (pindexPrev->nHeight <= BP3Y)
    {
        nAPR = BASE_POS_APR[nStakeColor];
    }
    else
    {
        nAPR = LTAPR;
    ...
    // compounds every time this currency stakes
    stSubsidy.nValue = (nAPR * (nSupply / 100)) / BPY;
...
unsigned int PoSNGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t nTargetSpacing = GetTargetSpacing(pindexLast->nHeight + 1, fProofOfStake);
...
{
    return PoSNGravityWave(pindexLast, fProofOfStake);
}
...
int64_t CTransaction::GetMaxWithdrawal(const MapPrevTx inputs)
{
    ...
    int64_t nTxMaxWD = 0;
    int64_t nCentUNIH = GetCent(PSN_COLOR_UNIH);
    ...
    for (unsigned int j = 0; j < vin.size(); j++)
    {
        ...
        if (txPrev.vout[prevout.n].nColor == PSN_COLOR_DPST)
        {
            ...
            if (!IsCoinStake())
            {
                ...
                if (nColor == nFeeColor)
                {
                    ...
                    if (IsWithdrawal())
                    {
                        ...
                        nTxFee = GetMaxWithdrawal(inputs) - GetValueOut(PSN_COLOR_UNIH);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (IsDeposit())
                    {
                        nTxFee = vValueIn[PSN_COLOR_UNIH] - GetValueOut(PSN_COLOR_DPST);
                        nTxChangeFee = 0;
                    }
                    else
                    {
...
bool CTransaction::ClientConnectInputs()
{
    ...
    // Take over previous transactions' spent pointers
    {
        ...
        for (oit = mapValuesOut.begin(); oit != mapValuesOut.end(); ++oit)
        {
              ...
              {
                  if (nOutputColor == PSN_COLOR_DPST)
                  {
                      if ((nOutputValue + GetMinFee(pindexPrev)) > vValueIn[PSN_COLOR_UNIH])
                      {
                          return false;
                      }
                  }
              ...
              {
                  if (nOutputColor == PSN_COLOR_UNIH)
                  {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int nStakeColor = (int) PSN_COLOR_P00;

    // *claimed* subsidy from GetProofOfStakeReward()
    int64_t nMintValue = 0;
    int nMintColor = (int) PSN_COLOR_P00;
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            for (int nIOColor = 1; nIOColor < N_COLORS; ++nIOColor)
            {
                ...
                if ((nIOColor == tx.GetFeeColor()) && pindex->pprev)
                {
                ...
                if (nFeeIO < nMinFeeIO)
                {
                    if (tx.IsDeposit(&mapInputs) && (nIOColor == PSN_COLOR_DPST))
                    {
                        ...
                        nIOFeeColor = PSN_COLOR_UNIH;
                    }
                    else if (tx.IsWithdrawal(&mapInputs) && (nIOColor == PSN_COLOR_UNIH))
                    {
                        // got here because outputs were UNH but inputs were DPT (ie no UNH in)
                        // fees are same as outputs
                        nFeeIO = tx.GetMaxWithdrawal(mapInputs) - mapValuesOut[PSN_COLOR_UNIH];
                        if (fDebug)
                        {
    ...
    if (IsProofOfWork())
    {
        ...
        struct AMOUNT reward = GetProofOfWorkReward(pindex->pprev, txCoinBase.vout[0].nColor);
        ...
        pindex->nStakeColor = (int) PSN_COLOR_P00;
    ...
    if (IsProofOfStake())
    {
        ...
        // posncoin: reward based nHeight and color
        struct AMOUNT reward = GetProofOfStakeReward(pindex->pprev, nStakeColor);

        // posncoin: reward is earned in the mint transaction, not the stake transaction
    ...
    for (int i = 1; i < N_COLORS; ++i)
    {
        ...
        if (i == PSN_COLOR_UNIH)
        {
            int64_t delta = vValueOut[PSN_COLOR_DPST] - vValueIn[PSN_COLOR_DPST];
    ...
    int64_t nTotalMintRewardPrev = (pindex->pprev ? pindex->pprev->vTotalMint[PSN_COLOR_UNIH] : 0);
    pindex->vTotalMint[PSN_COLOR_UNIH] = nTotalMintRewardPrev +
                                         pindex->vCoinbase[PSN_COLOR_UNIH] +
                                         vScavengedFees[PSN_COLOR_UNIH] -
                                         vFees[PSN_COLOR_UNIH];
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // but weight_not_atomic * GetCoin(color_not_atomic) -->
        //                                weight_atomic * 100 * GetCent(color_atomic)
	    // where weight is applied elsewhere (e.g. GetStakeWeightByColor).
        if (GetCoin(nColorIn) == GetCent(nColorIn))
        {
            bnCentSecond += 100 * CBigNum(nValueIn) * bnSeconds / GetCoin(nColorIn);
        }
        else
        {
            bnCentSecond += CBigNum(nValueIn) * bnSeconds / GetCent(nColorIn);
        }
...
bool CBlock::AcceptBlock()
{
    ...
    // PoS-N Coin: PoS does not start until PoW is finished
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PoS-N Coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Taxi Owners in Debt as Medallions Plummet in Value";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1505177520;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.strTxComment = "";
        ...
        txNew.SetColor((int) PSN_COLOR_P00);
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1505177520;
        ...
        assert(block.hashMerkleRoot ==
          uint256("0x71433b4d683c209edf8ed8f146056f8fbf2425bcb1da78ee49a88f27b83c0aaf"));
