...
static const unsigned int LAUNCH_TIME = 1505153717;
...
static const uint256 
     hashGenesisBlock("0x000119fdbf2d71d708bc01bf6343b9395c1b6afaaa8eacc47a2a6ab4d4a0fb0b");
static const uint256 
     hashGenesisBlockTestNet("0x000119fdbf2d71d708bc01bf6343b9395c1b6afaaa8eacc47a2a6ab4d4a0fb0b");
...
public:
    ...
    bool IsUncolored()
    {
        return (nColor == (int) PSN_COLOR_P00);
    }
...
public:
    ...
    {
        // posncoin: the coin mint transaction is marked with
        //    first input null
    ...
    */
    ...
    int64_t GetValueOut(int nColor) const
    {
        ...
        {
             throw std::runtime_error(strprintf("CTransaction::GetValueOut(): invalid color: %d", nColor));
        }
        int64_t nValueOut = 0;
    ...
    */
    int64_t GetNormalizedValueOut() const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, vout)
        {
            ...
            nValueOut += (BASE_COIN / GetCoin(txout.nColor)) * txout.nValue;
        }
        return nValueOut;
    }
    ...
    int GetColor() const
    {
        if (vout.size() == 0)
        {
              return (int) PSN_COLOR_P00;
        }
        if (IsDeposit())
        {
              return PSN_COLOR_DPST;
        }
        else if (IsWithdrawal())
        {
              return PSN_COLOR_UNIH;
        }
    ...
    int GetFeeColor(MapPrevTx const *pMapInputs = NULL) const
    {
        ...
        {
            nColor = PSN_COLOR_P00;
        }
        else if (IsDeposit())
        {
            nColor = PSN_COLOR_UNIH;
        }
        else if (IsWithdrawal())
        {
            nColor = PSN_COLOR_UNIH;
    ...
    //       - vout.size >= 2
    //       - vout[0].nColor == PSN_COLOR_PREM
    //       - other vouts nColor must be PSN_COLOR_DPST
    ...
    //     There is no need to prove the ownership of PREM for a withdrawal.
    //       - vout.size >= 2
...
public:
    ...
    // Total UNIH money is tracked for fee scavenging PoW
    ...
    CBlockIndex()
    {
        ...
        nChainTrust = 0;
        nCoinbaseColor = (int) PSN_COLOR_P00;
        nFlags = 0;
        bnStakeModifier = 0;
