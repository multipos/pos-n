...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid posncoin address: ")+input.get_str());
...
Value listdeposits(const Array& params, bool fHelp)
{
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
            {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                        string("Invalid posncoin address: ")+input.get_str());
            }
            if (setAddress.count(address))
            {
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        CWalletTx tempwtx;
        tempwtx.vout.push_back(CTxOut(nValue, PSN_COLOR_UNIH, scriptPubKey));
        ...
        entry.push_back(Pair("deposit amount",ValueFromAmount(nValue, PSN_COLOR_UNIH)));
        entry.push_back(Pair("max withdraw",ValueFromAmount(nMaxWD, PSN_COLOR_UNIH)));
        entry.push_back(Pair("currency", std::string(COLOR_TICKER[nColor])));
        entry.push_back(Pair("confirmations", out.nDepth));
        results.push_back(entry);
    }
...
Value createrawdeposit(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(Value& input, inputs)
    {
        ...
        if (tx.vout[nOutput].nColor == PSN_COLOR_UNIH)
        {
            nValueUNHIn += tx.vout[nOutput].nValue;
        }
        else if (tx.vout[nOutput].nColor == PSN_COLOR_PREM)
        {
    ...
    if ((!fCheck) && ((nValueDPTOut + nMinFee) > nValueUNHIn))
    {
        throw runtime_error(strprintf("Not enough fees. Minimim: %s\n",
                                      FormatMoney(nMinFee, PSN_COLOR_UNIH).c_str()));
    }
    ...
    if (fCheck)
    {
        return ValueFromAmount(nValueUNHIn - nMinFee, PSN_COLOR_UNIH);
    }
    else
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
...
Value createrawwithdrawal(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(Value& input, inputs)
    {
        ...
        if (tx.vout[nOutput].nColor == PSN_COLOR_DPST)
        {
    ...
    CTxOut out(0, PSN_COLOR_DPST, scriptMarker);
    rawTx.vout.push_back(out);
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                      string("Invalid posncoin address: ")+s.name_);
        }
    ...
    if (fCheck)
    {
        return ValueFromAmount(nMaxWD - nMinFee, PSN_COLOR_UNIH);
    }
    else
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
