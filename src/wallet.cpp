...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE)
    {
        ...
        std::string strAddr = CBitcoinAddress(redeemScript.GetID(), PSN_COLOR_P00).ToString();
...
std::pair<int, int64_t> CWallet::GetDebit(const CTxIn &txin, bool fMultiSig) const
{
    ...
    return std::make_pair(PSN_COLOR_P00, 0);
}
                         ...
                         int64_t& nValueRet, bool fMultiSig) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        else if (n < nTargetValue + GetCent(nColor))
        {
    ...
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + GetCent(nColor))
        ApproximateBestSubset(vValue, nTotalLower,
                                        nTargetValue + GetCent(nColor), vfBest, nBest, 1000);

    // If we have a bigger coin and (either the stochastic approximation didn't find a good solution,
    //                                   or the next bigger coin is closer), return the bigger coin
    if (coinLowestLarger.second.first &&
        ((nBest != nTargetValue && nBest < nTargetValue + GetCent(nColor)) ||
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nColor == nFeeColor)
                {
                    if ((nFeeRet < minTxFee) && (nChange > 0) && (nChange < (GetCent(nColor)/10)))
                    {
                ...
                // change
                //
                // posncoin: sub-tenth of a cent change is moved to fee
                if (nChange > (GetCent(nColor) / 10))
                {
                ...
                // fee delegate change
                //
                // posncoin: sub-tenth of a cent fee change is moved to fee
                if ((nColor != nFeeColor) && (nFeeChange > (GetCent(nFeeColor) / 10)))
                {
                    // Fill a vout to ourself
                    // TODO: pass in scriptChange instead of reservekey so
                    // change transaction isn't always pay-to-bitcoin-address
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CBitcoinAddress addr(ckid, PSN_COLOR_P00);
...
# if 0
        ...
        if (fDebug)
        {
            ...
            CBitcoinAddress coinAddress(keyID, PSN_COLOR_P00);
...
bool CWallet::GetStakeWeightByColor(int nColor, const CKeyStore& keystore, uint64_t& nWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        {
            nWeight += pcoin.first->vout[pcoin.second].nValue / GetCoin(nColor);
        }
    }
                                  ...
                                  int64_t nFees[], CTransaction& txMint, CTransaction& txStake, CKey& key)
{
    ...
    // Calculate PoS reward
    {
        ...
        // posncoin uses block rewards that do not depend on coin age
        uint64_t nCoinAge;
        CTxDB txdb("r");
    ...
    {
        txStake.vout[1].nValue = ((nStakeCredit / 2) / GetCent(nStakeColor)) * GetCent(nStakeColor);
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    NotifyAddressBookChanged(this, address, (int) PSN_COLOR_P00, "", fOwned, CT_DELETED);
