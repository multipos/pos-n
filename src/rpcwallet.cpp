...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "Returns a new posncoin address for receiving payments.\n");
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current posncoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <posncoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid posncoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress <posncoinaddress> <amount> [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <posncoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <posncoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "getreceivedbyaddress <posncoinaddress> [minconf=1] [multisig=false]\n"
            "Returns the total amount received by <posncoinaddress> in transactions with at least [minconf] confirmations.\n"
    ...
    if  (fAllColors)
    {
          ...
          for (int nColor = 1; nColor < N_COLORS; ++nColor)
          {
               obj.push_back(Pair(COLOR_TICKER[nColor],
                                     (double) vAmount[nColor] / (double) GetCoin(nColor)));
          }
          return obj;
    }
    else
    {
          return (double) vAmount[nDefaultCurrency] / (double) GetCoin(nDefaultCurrency);
    }
}
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toposncoinaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "[tx-comment] max size is " + strprintf("%d", MAX_TX_COMMENT_LEN) + "\n"
            "[product-id] is a positive integer\n"
	    "[narration] is a short (<=24 char) note"
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid posncoin address: ")+s.name_);

        if (nColor == (int) PSN_COLOR_P00)
        {
...
Value gettransaction(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->mapWallet.count(hash))
    {
        ...
        if (CheckColor(nFeeColor))
        {
            nTxFee = (wtx.IsFromMe(fMultiSig) ?
        ...
        if (wtx.IsFromMe(fMultiSig) && (nTxFee != 0))
        {
            Object objFee;
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; posncoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "validateaddress <posncoinaddress> [ticker]\n"
            "Return information about <posncoinaddress>.\n"
    ...
    CBitcoinAddress address(params[0].get_str());
    bool isValid;
    if ((nColor != (int) PSN_COLOR_P00) && (address.nColor != nColor))
    {
       isValid = false;
    }
    else
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <posncoinpubkey> [ticker]\n"
            "Return information about <posncoinpubkey>.");
    ...
    if (isValid)
    {
        CTxDestination dest = address.Get();
        string currentAddress = address.ToString();
        if (address.nColor == PSN_COLOR_P00)
        {
...
Value reservebalance(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        if (fReserve)
        {
            ...
            {
                nAmount = (nAmount / GetCent(nColor)) * GetCent(nColor);  // round to cent
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid PoS-N Coin Stealth Address."));
        return result;
    };
