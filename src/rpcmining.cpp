...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0) {
            struct AMOUNT subsidy = GetPoWSubsidy(pindexBest, nSubsidyColor);
...
#if PROOF_MODEL == PURE_POS
            if (nHeight < 1 || nHeight > nBestHeight || nHeight > nLastPoWBlock)
#else
            ...
            struct AMOUNT subsidy = GetPoWSubsidy(pblockindex, nSubsidyColor);
            ret.push_back(Pair("blockvalue", subsidy.nValue));
            ret.push_back(Pair("currency", COLOR_TICKER[subsidy.nColor]));
    }
    else {
            ...
            CBlock block;
            CBlockIndex* pblockindex = mapBlockIndex[hashBestChain];
            while (pblockindex->nHeight >= first)
            {
                pblockindex = pblockindex->pprev;
            }

           int i = first;
           while (pblockindex->nHeight < last)
           {
              Object obj;
              struct AMOUNT subsidy = GetPoWSubsidy(pblockindex, nSubsidyColor);
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    struct AMOUNT subsidy = GetPoWSubsidy(pindexBest, PSN_COLOR_P02);
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "posncoin is not connected!");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "posncoin is not connected!");
