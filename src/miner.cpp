...
bool SelectRandomMintColor(const CBlockIndex *pindexPrev, int& nMintColor)
{ 
    unsigned int nNumCurrencies = (unsigned int) PSN_COLOR_END - 1;
    ...
    {       
        ...
        {       
           ...
           if (it == setTried.end())
           {
               int r = std::rand() % PSN_COLOR_END;
               if (r == 0)
               { 
                  setTried.insert(i);
                  if (pindexPrev->vTotalMint[i] < GetMaxPoWMint(i))
                  {
                      nMintColor = i;
                      break;
                  }
        ...
        if (setTried.size() == nNumCurrencies)
        {
            break;
        }
    }
    return nMintColor != PSN_COLOR_P00;
}
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t pFees[])
{
    ...
    else
    {
        ...
        std::string strMineOnly = GetArg("-mineonly", COLOR_TICKER[PSN_COLOR_P00]);

        if (fDebug)
        {
            printf("CreateNewBlock(): -mineonly=%s\n", strMineOnly.c_str());
        }

        // if fail, default to PSN_COLOR_P00
        if (!GetColorFromTicker(strMineOnly, nMintColor))
        {
            printf("CreateNewBlock(): bad ticker for -mineonly: %s\n", strMineOnly.c_str());
        }
        else if (pindexPrev->vTotalMint[nMintColor] >= GetMaxPoWMint(nMintColor))
        {
            printf("CreateNewBlock(): no more %s (-mineonly) left to mine\n", strMineOnly.c_str());
            strMineOnly = GetArg("-mineonly2", COLOR_TICKER[PSN_COLOR_P00]);
            if (fDebug)
            {
                printf("CreateNewBlock(): -mineonly2=%s\n", strMineOnly.c_str());
            }
            if (!GetColorFromTicker(strMineOnly, nMintColor))
            {
                 printf("CreateNewBlock(): bad ticker for -mineonly2: %s\n", strMineOnly.c_str());
            }
            else if (pindexPrev->vTotalMint[nMintColor] >= GetMaxPoWMint(nMintColor))
            {
                printf("CreateNewBlock(): no more %s (-mineonly2) left to mine\n", strMineOnly.c_str());
                nMintColor = PSN_COLOR_P00;
            }
        }

        struct AMOUNT reward = GetProofOfWorkReward(pindexPrev, nMintColor);

        if (reward.nValue <= 0)
        {
           printf("CreateNewBlock(): 0 block reward for %s (-mineonly[2])\n", strMineOnly.c_str());
           nMintColor = PSN_COLOR_P00;
           if (!SelectRandomMintColor(pindexPrev, nMintColor))
           {
               printf("CreateNewBlock(): no more mineable currencies\n");
           }
           else
           {
               reward = GetProofOfWorkReward(pindexPrev, nMintColor);
           }
        }
    ...
    // Collect memory pool transactions into the block
    {
                                                          ...
                                                          mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (tx.IsDeposit())
            {
                nFeeInColor = PSN_COLOR_UNIH;
                nFeeOutColor = PSN_COLOR_DPST;
            }
            else if (tx.IsWithdrawal())
            {
                nFeeInColor = PSN_COLOR_DPST;
                nFeeOutColor = PSN_COLOR_UNIH;
            }
            else
            {
                nFeeInColor = nColor;
        ...
        while (!vecPriority.empty())
        {
            ...
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) ||
                 (dPriority < (PRIORITY_MULTIPLIER[nFeeCollectColor] * GetCoin(nFeeCollectColor) * 144 / 250))))
            {
                fSortedByFee = true;
            ...
            for (int i = 1; i < N_COLORS; ++i)
            {
                ...
                if (tx.IsDeposit())
                {
                    if (i == PSN_COLOR_UNIH)
                    {
                        txfee = tx.GetValueIn(mapInputs, PSN_COLOR_UNIH) -
                                tx.GetValueOut(PSN_COLOR_DPST);
                    }
                }
                else if (tx.IsWithdrawal())
                {
